...
## 无背景选项 + 背景可读性 + 专注页横屏适配（2026-09-14，1.4.8 / versionCode 27）
- 背景新增「无背景」：纯深色底，无照片无渐变；设置页与准备页均可选。
- 图片背景遮罩加深（黑 62% / 白 72%），以最差对比图片也能读清文字为准；主题文字色不变。
- 专注页横屏适配：宽>高时双栏布局（准备页=任务与时长在左、模式与开始按钮在右；运行页=计时环在左、控制在右），原竖屏单列不变；≥840dp 的侧栏仅在竖屏宽屏出现，横屏由双栏利用宽度。

## 默认纸白背景 + 横屏锁定按钮 + 横屏布局收窄居中（2026-09-14，1.4.9 / versionCode 28）
...
## 极致模式确认弹窗 + 停止等待期弹窗循环（2026-09-14，1.4.11 / versionCode 30）
- 新增应用内「开始极致专注？」确认弹窗（确认后屏幕固定、无法退出、系统授权框提示）；极致任务从任务卡进入也先落准备页再确认，不再跳过确认直开。extremeRound 测试更新为经确认流程。
- 修复"没点确定时系统授权框循环弹出"：等待用户确认系统「应用固定」授权期间改为 15s 低频重试（此前 3s 重发在对话框未决时表现为循环弹框）；确认并锁定过之后失锁仍 3s 快速重锁。hasPinnedOnce 状态区分两阶段。

## 屏幕固定预授权（2026-09-14，1.4.12 / versionCode 31）
- 防护设置向导新增「预授权屏幕固定」：点击固定 2.5 秒后自动解除，完成系统一次性授权记忆。此后开始极致专注只剩应用内确认弹窗，不再弹系统框。极致确认弹窗文案同步更新（提示可在防护设置提前授权）。

## 预授权时序修复 + 确认后 250ms 快速重锁（2026-09-14，1.4.13 / versionCode 32）
- 修复预授权流程 bug：固定延时解除会在用户尚未点「开始使用」时失效，之后确认会困在固定状态。改为轮询检测到真正锁定后再延时 1 秒解除（60s 超时），按钮显示等待/已固定/完成/超时状态。
- 授权记忆完成后（hasPinnedOnce）重锁节流 3s → 250ms：此后 startLockTask 不再弹系统框，无闪烁风险，系统手势的逃离窗口压到 0.25 秒。
...
## 严格拉回弹窗 + 软性中断统计落地（2026-09-15，1.6.0 / versionCode 38）
