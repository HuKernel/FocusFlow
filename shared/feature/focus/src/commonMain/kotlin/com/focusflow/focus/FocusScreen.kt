            ...
            Row(Modifier.fillMaxSize()) {
                Column(Modifier.weight(1f).fillMaxHeight()
                    ...
                    Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                        ...
                        if (!extremeRunning) FocusTextButton(onClick = onBack) { Text("返回任务", color = theme.secondary) }
                        Spacer(Modifier.weight(1f))
                        if (onToggleLandscape != null) FocusTextButton(onClick = {
                            landscapeLocked = !landscapeLocked
                            onToggleLandscape?.invoke(landscapeLocked)
                        }) { Text(if (landscapeLocked) "竖屏" else "横屏", color = theme.secondary) }
                            ...
                            when {
                                running -> {
                                    if (extreme) {
                                        ...
                                        enabled = !state.busy,
                                        icon = if (phase == TimerState.PAUSED) Icons.Filled.PlayArrow else Icons.Filled.Pause,
                                        modifier = Modifier.testTag("pause_resume")
                                    ) {
                                        Text(if (phase == TimerState.PAUSED) "继续专注" else "暂停", color = androidx.compose.ui.graphics.Color.White)
                                    }
                                    if (stopwatch) FocusSecondaryButton(
                                        onClick = { model.complete(active.session.id) },
                                        enabled = !state.busy,
                                        modifier = Modifier.testTag("complete_focus")
                                    ) {
                                        Text("完成专注", color = FocusColors.Primary)
                                    }
                                    if (!extreme) FocusSecondaryButton(
                                        onClick = { cancelling = true },
                                        enabled = !state.busy
                                    ) {
                                        Text("取消本次专注", color = FocusColors.Primary)
                                    }
                                ...
                                phase == TimerState.BREAKING -> {
                                    ...
                                    FocusSecondaryButton(
                                        onClick = { model.skipBreak(active.session.id) },
                                        enabled = !state.busy
                                    ) {
                                        Text("结束休息", color = FocusColors.Primary)
                                    }
                                }
                                else -> {
                                    ...
                                    if (celebrated) FocusPrimaryButton(
                                        onClick = { feedback.play(SoundEvent.BREAK_START); model.startBreak(active.session.id) },
                                        enabled = !state.busy
                                    ) {
                                        Text("休息 ${active.breakDuration / 60_000} 分钟", color = androidx.compose.ui.graphics.Color.White)
                                    }
                                    FocusSecondaryButton(
                                        onClick = { onGuardFocusEnded?.invoke(); model.dismiss(active.session.id) },
                                        enabled = !state.busy,
                                        modifier = Modifier.testTag("dismiss_focus")
                                    ) {
                                        Text("结束本轮", color = FocusColors.Primary)
                                    }
                                }
                            }
                        }
                    ...
                    state.error?.let { Text(it, color = MaterialTheme.colorScheme.error); FocusTextButton(onClick = model::retry) { Text("重试恢复", color = FocusColors.Primary) } }
                }
